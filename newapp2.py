...
if uploaded_file:
    ...
    if document:
        ...
        if summary:
            ...
            append_mode = st.checkbox("Append to existing summary CSV file")

            if append_mode:
                existing_csv = st.file_uploader("Select existing CSV to append to", type=["csv"])
                if existing_csv:
                    try:
                        ...
                        st.download_button(
                            ...
                            data=combined_csv,
                            file_name="updated_receipt_summary.csv",
                            mime="text/csv"
                        )
                    except Exception as e:
                        st.error(f"❌ Failed to append: {e}")
                else:
                    st.info("Please select a CSV file to append to.")
            else:
